...
class info_post(Session):
    def __init__(self, url: str):
        super().__init__()
        if not findall(r'[0-9]{1,}$', url):
            raise InvalidUrl()
        self.id = findall(r'[0-9]{1,}$', url)
    ...
    def download(self, out:Optional = None, chunk_size: int = 1024) -> Union[None, BytesIO]:
        request = self.get(self.aweme['aweme_detail']['video']['download_addr']['url_list'][0], stream=True)
